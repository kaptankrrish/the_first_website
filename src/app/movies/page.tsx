        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <motion.div
              ...
              className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-red-500/20 via-rose-500/20 to-pink-500/20 border border-white/10 flex items-center justify-center shrink-0 shadow-[0_0_24px_rgba(244,63,94,0.2)]"
            >
              <Film className="w-5 h-5 sm:w-6 sm:h-6 text-red-200" />
            </motion.div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                <span className="px-2 py-0.5 rounded-full bg-red-500/15 text-red-300 text-[10px] font-semibold uppercase tracking-[0.18em] border border-red-400/20 inline-flex items-center gap-1.5">
                  <span className="relative flex h-1.5 w-1.5">
                    <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-red-400 opacity-75" />
                    <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-red-400 shadow-[0_0_6px_rgba(248,113,113,0.8)]" />
                  </span>
                  Trending
                </span>
              ...
              <p className="text-sm text-muted-foreground/80 mt-1.5 max-w-2xl text-pretty">
                Trending films, search any title, and build your personal watchlist.
              </p>
            </div>
          </div>
          <div className="relative w-full sm:w-80 shrink-0">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-red-300" />
            <Input
              placeholder={t.movies.searchMovies}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-10 glass-strong border-white/10 focus-visible:border-red-400/40"
            />
          </div>
        </div>
