    ...
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <motion.div
              ...
              className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-sky-500/20 via-blue-500/20 to-cyan-500/20 border border-white/10 flex items-center justify-center shrink-0 shadow-[0_0_24px_rgba(56,189,248,0.2)]"
            >
              <Cloud className="w-5 h-5 sm:w-6 sm:h-6 text-sky-200" />
            </motion.div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                <span className="px-2 py-0.5 rounded-full bg-sky-500/15 text-sky-300 text-[10px] font-semibold uppercase tracking-[0.18em] border border-sky-400/20 inline-flex items-center gap-1.5">
                  <span className="relative flex h-1.5 w-1.5">
                    <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-sky-400 opacity-75" />
                    <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-sky-400 shadow-[0_0_6px_rgba(56,189,248,0.8)]" />
                  </span>
                  Live
                </span>
              ...
              <p className="text-sm text-muted-foreground/80 mt-1.5 max-w-2xl text-pretty">
                Real-time forecasts, hourly outlook, and 7-day predictions for any city.
              </p>
            </div>
          </div>
        </div>
      ...
      <div ref={searchRef} className="relative max-w-md">
        <div className="relative">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-sky-300" />
          <Input
            placeholder={t.weather.searchCity}
            value={searchQuery}
            onChange={(e) => handleSearchChange(e.target.value)}
            onFocus={() => { if (searchResults.length > 0) setShowDropdown(true); }}
            className="pl-10 glass-strong border-white/10 focus-visible:border-sky-400/40"
          />
        </div>
        <AnimatePresence>
